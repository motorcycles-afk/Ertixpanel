...
function initializeTerminal() {
    ...
    // Fit terminal and send resize information
    setTimeout(() => {
        fitAddon.fit();
        const dimensions = fitAddon.proposeDimensions();
        if (websocket && websocket.readyState === WebSocket.OPEN && dimensions) {
        ...
        websocket.onmessage = (event) => {
            try {
                ...
                switch (data.type) {
                    ...
                    case 'data':
                        if (terminal) {
                            // Handle data more carefully to prevent glitches
                    ...
                    case 'exit':
                        showNotification('Console session ended', 'warning');
                        disconnectConsole();
                        break;
